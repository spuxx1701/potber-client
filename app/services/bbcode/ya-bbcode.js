  ...
  #contentLoop = function (tagsMap, content) {
    for (const tag of tagsMap) {
      ...
      if (!this.contentModules[module.type]) {
        throw new Error(
          'Cannot parse content block. Invalid block type [' +
            module.type +
            '] provided for tag [' +
            tag.module +
            ']'
  ...
  #tagLoop = function (tagsMap, parent) {
    ...
    while (currentTagIndex < tagsMap.length) {
      ...
      if (
        ...
        tagsMap[currentTagIndex].isClosing
      ) {
        currentTagIndex++;
      ...
      if (found !== false) {
        tagsMap[currentTagIndex].closing =
          tagsMap[tagsMap[currentTagIndex].matchTag];
      } else {
      ...
      // sweep children
      if (childStart < found) {
        tagsMap[currentTagIndex].children = tagsMap
          .slice(childStart, found)
          .map((child) => {
